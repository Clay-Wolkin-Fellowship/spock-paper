...
% Do we want to mention Belady's algorithm specfically picks the current item in cache that would be receached last?
In general we measure the Time to Recache for each evicted element as a qualitative measurement of a caching policies performance.
...
We believe that these metrics will not only be useful for comparing the performance of two different caching policies,
  but allow designers of such policies to indentify memory access patterns that the current policy fares poorly with.
